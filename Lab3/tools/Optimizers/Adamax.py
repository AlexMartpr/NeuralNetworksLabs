...
class Adamax:
    ...
    def run(self, input, output):
        ...
        # U_b = np.zeros_like(self.model.b)
        ...
        for e in range(self.model.epochs):
            ...
            for mini_batch in mini_batches:
                ...
                for l in range(self.model.layers):
                    ...
                    # U_w[l] = np.array([max(self.b2 * U_w[l][i], abs(w_grad[l][i]) * eps) for i in range(self.model.N)])
                    # print(w_grad[l])
                    h = np.max(abs(w_grad[l]))
                    U_w[l] = max(self.b2 * U_w[l], h)
                    # print(U_w[l])
                    # exit(-1)
                    # U_b[l] = max(self.b2 * U_b[l], abs(b_grad[l]) * eps)
