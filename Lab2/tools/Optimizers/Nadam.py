        ...
        for e in range(self.model.epochs):
            ...
            for mini_batch in mini_batches:
                ...
                for l in range(self.model.layers):
                    ...
                    k_w = self.b1 * m_h_w + (1 - self.b1) * w_grad[l] / (1 - self.b1 ** t)
                    k_b = self.b1 * m_h_b + (1 - self.b1) * b_grad[l] / (1 - self.b1 ** t)
                    self.model.w[l] += self.model.lr * k_w  / (np.sqrt(v_h_w) + eps)
                    self.model.b[l] += self.model.lr * k_b  / (np.sqrt(v_h_b) + eps)
